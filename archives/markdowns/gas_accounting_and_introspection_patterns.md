...
* **precisely allot gas to their sub-calls**, via the `gas` argument of `CALL*` opcodes (`CALL, DELEGATECALL, STATICCALL, CALLCODE`); and
...
the gas argument of `CALL*` opcodes may be used to either limit the amount of gas handed to a sub-call, or to reserve some gas for the CALLER’s purposes. the former is typically implemented as a reentrancy defense pattern, which has been discouraged post-Dencun; while the latter remains widely supported.

hence, our concern is figuring out the frequency of contracts that use `requested_gas` to reserve a buffer >1/64 of their `remaining_gas` , and separately, the frequency of contracts that use `requested_gas` to pass <63/64 of their `remaining_gas` to a sub-call.

* **BLOCKER**: figure out how to check if GSWC is `requested_gas` or `all_but_one_64th`
...
* checking the amount of gas they have before and after performing a call and noting the difference
...
* checking that the amount of gas they were allotted by a CALLER is above some threshold that would be enough to execute some sub-call, and throwing an error if not
    
    * contracts such as smart contract wallets can use this pattern to defend against the insufficient gas grieving attack described in [Ethereum, the Concept of Gas, and its Dangers](https://ronan.eth.limo/blog/ethereum-gas-dangers/).